...
const expectedIncludeString = "&include=field_media_image,field_media_audio_file";
...
test("Podcast Episode DataType Testing", (done) => {
  fixtureData.getFixtureData(expectedIncludeString).then((mockResponse) => {
    for (const key in mockResponse.data) {
      ...
      if (origData.thumbnail !== undefined && origData.thumbnail?.data === undefined) {
      ...
      if (
        ...
        expect(audioFile).not.toBe(null);
