...
export const SlideDisplayFullWidthOneColumn : React.FunctionComponent = (props: SlideDisplayProps) => {
  ...
  const [ backgroundImage, setBackgroundImage ] = useState(data.field_background_image);

  if (!backgroundImage.hasData()) {
    const ecp = new EntityComponentProps(backgroundImage);
    ecp.getData(backgroundImage.getIncluded())
      .then(res => res.json())
      .then(ajaxData => {
  ...
  // ========== STYLES ==========
  let rowStyle = {"backgroundColor":`${data.field_background_color?.color}`};
  let backgroundVersion = window.matchMedia('min-width: 700px').matches ?
    `url('${backgroundImage.image_style_uri.getStyleByMachineName('fullscreen')}')` :
    `url('${backgroundImage.image_style_uri.getStyleByMachineName('thumbnail')}')`
  ...
  if (data.field_slide_text?.length) {
    for (var key in data.field_slide_text) {
      textLines.push((
        <p
          key={textLines.length + 1}
          className={data.field_slide_text[key]['key']}
          style={{'color': `${data.field_text_color?.color}`}}>{data.field_slide_text[key]['value']}</p>
      ));
    }
  }
