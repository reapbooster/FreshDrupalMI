...
const ProgramDaySession: React.FC<ProgramDaySessionProps> = (props: ProgramDaySessionProps) => {
    ...
    const renderSessionTime = (session: any) => {
        let startTime = moment(session.field_panel_start_time, 'hh:mm').format('H:mm');
    ...
    const titleNode = <h5 dangerouslySetInnerHTML={{ __html: _.unescape(session.title) }}></h5>;

    const summary = <p dangerouslySetInnerHTML={{ __html: session?.field_description }} />;

    const renderSpeakers = (session: any) => {
        if (session?.field_speakers && session?.field_speakers.length > 0) {
            const speakerIds = session.field_speakers.split(',').map((id: any) => parseInt(id));
