...
const GridEventsSpeakers: React.FunctionComponent = (
  ...
  if (!fetchRan) {
    ...
    // Fetch Main Content
    fetch(
      "/api/v1.0/grid_speakers?_format=json&sort_order=ASC&items_per_page=1000&eventid=" +
        grid_id
    )
      .then((res) => res.json())
      .then((incoming) => {
  ...
  return (
    <MainContainer className="container py-5">
      <Row>This is the Grid Speakers Full component for Grid ID: {grid_id}</Row>
      <Row>
        {fetchedData.map((item, key) => {
          let imagePath =
            item.field_biopic == null || item.field_biopic == "null"
              ? "/sites/default/files/styles/large/public/Missing%20Photo_0.jpg"
              : "https://grid.milkeninstitute.org/events/speakers/" +
                item.field_biopic;
