 ...
 * @code
 * field_promo_slide:
 *   plugin: milken_migrate:promo_slide
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    try {
      ...
      if ($file instanceof FileInterface) {
        ...
        if (file_exists($realpath)) {
          $destination['field_text_color'] = ['color' => @$this->generateComplimentaryColorFromImageHistorgram($realpath)];
        }
      }
    ...
    try {
      ...
      if ($slide instanceof EntityInterface) {
        $slide->save();
        $row->setDestinationProperty($destination_property, ['entity' => $slide]);
        return ['entity' => $slide];
      }
    }
    catch (\Exception $e) {
